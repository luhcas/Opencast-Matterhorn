 ...
 * <p/>
 * Contains code copied from class <code>appetizer.util.Assert</code> of project
...
public class StringSupport {
  ...
  /**
   * Removes all whitespace from the beginning and end and collapses multiple
   * whitespace to a single one.
   * 
  ...
  /**
   * Schreibt das Wort klein. Abkürzungen wie "AGB" oder "URL" werden nicht
   * kleingeschrieben.
   * 
   * @param s
   *          darf null sein
  ...
  /**
   * Begrenzt die Länge das Strings auf <i>maxSize</i>. Ist <i>s</i> null, wird
   * null zurückgegeben.
   */
   ...
   * 
   * @param maxSize
   *          die maximale Länge des Nutzstrings (also ohne die
   *          Auslassungszeichen <i>ellipsis</i>)
   * @param cutEnd
   *          true: Der String wird hinten beschnitten; false: Vorne
   */
  public static String limit(String s, int maxSize, boolean cutEnd,
      String ellipsis) {
    if (s != null && s.length() > maxSize) {
  ...
  /**
   * Siehe {@link #limit(String,int,boolean,String)}. <i>ellipsis</i> == "...";
   * <i>cutEnd</i> == true
   */
  ...
  /**
   * Stellt dem String <i>s</i> den String <i>prepend</i> voran, falls dieser
   * nicht bereits mit selbigem beginnt. Ist <i>s</i> null wird null
   * zurückgegeben.
   */
  ...
  public static String camelCase(String string, char separator,
      boolean capitalize) {
    if (string == null)
      return string;
  ...
  /**
   * Verknüpft die Stringrepräsentationen der Objekte <i>obj</i> mit dem
   * String <i>chain</i>.
   * 
   * @param chain
   *          Glied oder null
  ...
  /**
   * Liefert den String <i>s</i> versehen mit dem Präfix <i>prefix</i> falls
   * <i>s</i> nicht leer ist, sonst <i>s</i>.
   * <p/>
   * Beispiel: prefix(null, "pre") -> null; prefix("x", "pre") -> "prex";
   * prefix("", "pre") -> "";
   */
  public static String prefix(String s, String prefix) {
    return !Tool.empty(s) ? prefix + s : s;
  }
  ...
  /**
   * Zerlegt einen String. (<code>tokens == null</code>,
   * <code>trim == true</code>, <code>greedyLast == false</code>,
  ...
  public static String[] tokenize(String s, String delim, int flags) {
    return tokenize(s, delim, null, (flags & TRIM) > 0,
        (flags & STRIP_EMPTY) > 0, (flags & GREEDY_LAST) > 0,
        (flags & KEEP_DELIM) > 0);
  }

  /**
   * Zerlegt einen String. (<code>trim == true</code>,
   * <code>greedyLast == false</code>, <code>keepDelim ==
   * false</code>)
   * 
   * @return den zerlegten String <code>s</code> als Array oder
   *         <code>null</code> falls <code>s == null</code>
   * @see #tokenize(String,String,String[],boolean,boolean,boolean,boolean)
   */
  ...
  /**
   * Zerlegt einen String. (<code>greedyLast == false</code>,
   * <code>keepDelim == false</code>)
   * 
   * @return den zerlegten String <code>s</code> als Array oder
   *         <code>null</code> falls <code>s == null</code>
   ...
   */
  public static String[] tokenize(String s, String delim, String[] tokens,
      boolean trim, boolean greedyLast) {
    return tokenize(s, delim, tokens, trim, false, greedyLast, false);
  }

  public static String[] tokenize(String s, String delim, String[] tokens,
      int flags) {
    return tokenize(s, delim, tokens, (flags & TRIM) > 0,
        (flags & STRIP_EMPTY) > 0, (flags & GREEDY_LAST) > 0,
        (flags & KEEP_DELIM) > 0);
  }

  /**
   * Zerlegt den String <code>s</code> anhand der Trennzeichen im String
   * <code>delim</code> und übergibt das Ergebnis im Array <code>tokens</code>.
   * Falls <code>tokens == null</code> ist, oder kleiner als erforderlich, so
   * wird ein passendes neues Array erzeugt. Ist <code>trim == true</code> so
   * werden Whitespaces vor und nach einem Token abgeschnitten. Wenn
   * <code>greedyLast == true</code> und <code>tokens</code> zu klein ist, so
   * schluckt das letzte Token den ganzen Rest (mit evtl. noch vorkommenden
   * Trennzeichen). Ist <code>keepDelim == true</code> werden die Trennzeichen
   * ebenfalls zurückgegeben.
   * <p/>
   * Um die Performance zu erhöhen sollten die Trennzeichen absteigend nach der
   * zu erwarteten Häufigkeit geordnet werden, d. h. das häufigste nach vorne
   * 
   * @return den zerlegten String <code>s</code> als Array oder
   *         <code>null</code> falls <code>s == null</code>
   */
  public static String[] tokenize(String s, String delim, String[] tokens,
      boolean trim, boolean stripEmpty, boolean greedyLast, boolean keepDelim) {
    ...
    if (delim_len == 0 || s_len == 0) {
      ...
      int start_offset = 0;
      for (int offset = 0; offset <= s_len
          && tokens_list.size() != token_max_count; offset++) {
        ...
        if (offset < s_len) {
          ...
          // wenn kein Trennzeichen erkannt worden ist, nächstes Zeichen
          // prüfen
