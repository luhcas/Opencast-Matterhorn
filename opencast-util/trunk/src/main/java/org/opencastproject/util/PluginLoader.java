...
public class PluginLoader {
	...
	/**
	 * Search all loadable classes that match the given criteria (package
	 * name and subclass relations).
	 * 
	 * @param pkg The package name, the classes must belong to. Subpackages
	 * of the specified package are not searched.
	 * @param ext The class name of any superclass of the classes found.
	 * Superclasses are searched recursively.
	 * @exception IllegalArgumentException signals a missing package name.
	...
	/**
	 * Search all loadable classes that match the given criteria (package
	 * name and multiple implements relations).
	 * 
	 * @param pkg The package name, the classes must belong to. Subpackages
	 * of the specified package are not searched.
	 * @param impl The name of interfaces that must be implemented by the
	 * class. Specify <code>null</code> if the class doesn't have to implement
	...
	/**
	 * Search all loadable classes that match the given criteria (package
	 * name, subclass relation and multiple implements relations). Currently
	 * only the following classes from the classpath can be found:<br>
	 * <ul>
	 ...
	 * class. Specify <code>null</code> if the class doesn't have to implement
	 * any specific interface.
	 * @param cl a classloader responsible for locating and loading the classes.
				...
				if (!classes.containsKey(res[i]))
					try {
						if (cl != null)
							classes.put(
								res[i],
								Class.forName(pkg.concat(".").concat(res[i]),
									true, cl));
						else
							classes.put(
								res[i],
	 ...
	 * 
	 * @param url the URL where all available classes should be retrieved.
	...
	private static String[] findClassNames(URL url) {
		...
		if (url.getProtocol().equals("file")) {
			...
			try {
				archiveFile =
					new URL(
						url.getFile().substring(0, url.getFile().indexOf('!')));
			...
			try {
				archiveName =
					java.net.URLDecoder.decode(archiveFile.getFile(), "UTF-8");
			} catch (java.io.UnsupportedEncodingException e) {
				System.err.println(e);
