...
/**
 * This class represents the mime type registry that is responsible for providing resolving
 * mime types through all system components.
 * <p>
 * The registry is initialized from the file <code>MimeTypes.xml</code> which is located
 * in the package <code>ch.ethz.chid.mms.pd.replay.common</code>.
 * 
...
public class MimeTypes {
	...
	static void initFromFile() {
		try {
			...
			log_.error("Error initializing mime type registry: definition file not found!");
		} catch (IOException e) {
			log_.error("Error initializing mime type registry: " + e.getMessage());
		} catch (ParserConfigurationException e) {
			log_.error("Configuration error while parsing mime type registry: " + e.getMessage());
	...
	/**
	 * Initializes the mime type registry with those types that
	 * are relevant for replay, e. g. <code>video/dv</code> and 
	 * <code>video/mj2</code>.
	 */
	 ...
	 * <code>video/mj2</code>.
	 * <p>
	 * If no mime type can be derived a <code>UnknownFileTypeException</code>
	 * is thrown.
	 * 
	 ...
	 */
	public static MimeType parseMimeType(String mimeType) throws UnknownFileTypeException {
		if (mimeType == null)
	 ...
	 * <p>
	 * For example, if the suffix is <code>mj2</code>, the
	 * mime type will be that of a ISO Motion JPEG 2000 document.
	 * <p>
	 ...
	 * @return the corresponding mime type
	 * @throws UnknownFileTypeException if the suffix does not map to a mime type
	 */
	 ...
	 * <p>
	 * This method tries various ways to extract mime type information
	 * from the files name or its contents.
	 * <p>
	 * If no mime type can be derived from either the file name or its
	 * contents, a <code>UnknownFileTypeException</code> is thrown.
	 * 
	 * @param file the file
	 * @return the corresponding mime type
	 * @throws UnknownFileTypeException if the mime type cannot be derived from the file
	 */
	public static MimeType fromFile(File file) throws UnknownFileTypeException, IOException {
		if (file == null)
		...
		if (separatorPos > 0 && separatorPos < filename.length() - 1) {
			...
			throw new UnknownFileTypeException("Unable to get mime type without suffix", file);
		}
...
//		}
		...
		throw new UnknownFileTypeException("File '" + file.getAbsolutePath() + "' cannot be matched to any mime type");
	}
