...
public class YouTubeDeliveryAction extends DeliveryAction {
     ...
     *
     * @param entry_url URL for video entry
     */
     ...
     *
     * @param entry request VideoEntry
     * @return created VideoEntry
     */

    private VideoEntry uploadVideo(VideoEntry entry) {
     ...
     *
     * @param file_name name of file ending in video extension
     * @return mime type
     */
    ...
    /**
     * Creates an initialized YouTubeMediaGroup to be used to post clip
     * metadata.
     *
     * @param entry VideoEntry
    ...
    private YouTubeMediaGroup createMediaGroup(VideoEntry entry) {
        ...
        // Category
        media_group.addCategory(new MediaCategory(YouTubeNamespace.CATEGORY_SCHEME,
     ...
     *
     * @param media_group YouTubeMediaGroup specifying clip metadata
     ...
     *
     * @param entry VideoEntry to be posted
     * @return ceaated video entry
     */

    private VideoEntry insertEntry(VideoEntry entry) {
        try {
            log("Starting upload of " + getMediaPath());
            VideoEntry created_entry = service.insert(
                    new URL(configuration.getUploadUrl()), entry);
    ...
    /**
     * Parses the error XML in an InvalidEntryRequest. Attempts to set the
     * action status to a string rendering of the error. Retrns a
     * FailedException or RetryException with error information depending
     * on the severity of the error code.
     *
     * @param except InvalidEntryException
     * @return FailedException of RetryException
     */

    private RuntimeException decodeEntryException(InvalidEntryException except) {
     ...
     *
     * @param domain YouTube error "domain" or class
     * @param code YouTube error code
     * @param location YouTube error location (typically xpath to service request)
    ...
    /**
     * Returns a YouTube error "location" as a String if recognized. Returns
     * the original location, otherwise.
     *
     * @param location typically an xpath to the data in the service request
     ...
     *
     * @param entry VideoEntry to be added
     * @param playlist_id YouTube id of destination playlist
     */

    private void addToPlaylist(VideoEntry entry, String playlist_id) {
        try {
        ...
        while(true) {
            ...
            if (!checked_entry.isDraft()) {
                ...
                throw new FailedException(
                        getMediaPath() + ": " + publication_state.getDescription());
            }
            else {
                try {
    ...
    private VideoEntry refreshEntry() {
        try {
