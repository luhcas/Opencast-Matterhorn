...
/**
 * Implements the media package timeline.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id: TimelineImpl.java 2905 2009-07-15 16:16:05Z ced $
 */
public class TimelineImpl extends XMLCatalog implements Timeline {
  ...
  final static Logger log_ = LoggerFactory.getLogger(TimelineImpl.class.getName());
   ...
   */
  private TimelineImpl(File file) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
  ...
  /**
   * Creates a new timeline file at the specified location for a media package with the given identifier.
   * 
   ...
   *           if the md5 checksum cannot be computed
   */
  static TimelineImpl newInstance(MediaPackage mediaPackage) throws IOException, UnknownFileTypeException,
          ParserConfigurationException, TransformerException, NoSuchAlgorithmException {
    TimelineImpl timeline = new TimelineImpl(new File(mediaPackage.getRoot(), MediaPackageElements.TIMELINE_FILENAME));
    timeline.save();
    return timeline;
  }
   ...
   *           if the md5 checksum cannot be computed
   */
  public static TimelineImpl newInstance() throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
    String[] name = MediaPackageElements.TIMELINE_FILENAME.split("\\.");
    File file = File.createTempFile(name[0], "." + name[1]);
    return new TimelineImpl(file);
  }

  /**
   * Reads a timeline from the specified file and returns it encapsulated in a {@link Timeline} object.
   * 
   * @param file
   *          the timeline file
   ...
   */
  public static TimelineImpl fromFile(File file) throws IOException, UnknownFileTypeException,
