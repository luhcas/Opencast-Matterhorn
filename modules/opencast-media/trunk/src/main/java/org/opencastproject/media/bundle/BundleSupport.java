...
public class BundleSupport {
   ...
   * <li><code>Replace</code> replaces elements in the target bundle with
   * matching identifier</li>
   * <li><code>Skip</code> skips elements from the source bundle with matching
   * identifer</li>
   ...
   *          the bundle directory
   * @param force
   *          <code>true</code> to relock an already locked bundle
   * @throws BundleException
   *           if the lock already exists or cannot be created
   */
  public static boolean lockBundle(File dir, boolean force)
      throws BundleException {
    File lock = new File(dir, Bundle.LOCKFILE);
    if (lock.exists()) {
      if (force)
    ...
    try {
      ...
      throw new BundleException("Error creating bundle lock at " + dir + ": "
          + e.getMessage());
    }
    return true;
  }
  ...
  public static void unlockBundle(File dir) throws BundleException {
    ...
    try {
      ...
      throw new BundleException("Error removing bundle lock at " + dir + ": "
          + e.getMessage(), e);
    }
  }
   ...
   * 
   * @param targetDir
   *          the target bundle directory
   * @param sourceDir
   *          the source bundle directory
   * @param move
   *          <code>true</code> to move the bundle contents, <code>false</code>
   *          to make a copy
   * @param mode
   *          conflict resolution strategy in case of identical element
   *          identifier
   * @throws BundleException
   *           if an error occurs either accessing one of the two bundles or
   *           merging them
   */
  public static Bundle merge(File targetDir, File sourceDir, boolean move,
      MergeMode mode) throws BundleException {
    Bundle src = null;
    Bundle dest = null;
    ...
    try {
      ...
      try {
        ...
        throw new BundleException("Error loading source bundle: "
            + e.getMessage());
      }
      ...
      try {
        ...
        throw new BundleException("Error loading target bundle: "
            + e.getMessage());
      }
        ...
        for (BundleElement e : src.elements()) {
          if (dest.getElementById(e.getIdentifier()) == null)
            ...
            if (MergeMode.Replace == mode) {
              log_.debug("Replacing element " + e.getIdentifier()
                  + " while merging " + dest + " with " + src);
              dest.remove(dest.getElementById(e.getIdentifier()));
              dest.add(e, move);
            } else if (MergeMode.Skip == mode) {
              log_.debug("Skipping element " + e.getIdentifier()
                  + " while merging " + dest + " with " + src);
              continue;
            } else if (MergeMode.Merge == mode) {
              log_.debug("Renaming element " + e.getIdentifier()
                  + " while merging " + dest + " with " + src);
              e.setIdentifier(null);
              dest.add(e, move);
            } else if (MergeMode.Fail == mode) {
              throw new BundleException("Target bundle " + dest
                  + " already contains element with id " + e.getIdentifier());
            }
          }
        }
        dest.save();
