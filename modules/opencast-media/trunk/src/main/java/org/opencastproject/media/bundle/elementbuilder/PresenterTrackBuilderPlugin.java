 ...
 * movie tracks and provides the functionality of reading it on behalf of the
 * bundle.
 * <p>
 ...
 * @version $Id: PresenterTrackBuilderPlugin.java 102 2009-04-03 20:05:36Z
 *          wunden $
 */
public class PresenterTrackBuilderPlugin extends AbstractTrackBuilderPlugin
  ...
  private final static Logger log_ = LoggerFactory
      .getLogger(PresenterTrackBuilderPlugin.class);
  ...
  public boolean accept(File file, BundleElement.Type type,
    ...
    if (type == null || flavor == null)
      return false;
        ...
        .equals(PresenterTrack.FLAVOR))
        && !checkFilenamePrefix(file, PresenterTrack.FILENAME_PREFIX))
      return false;
  ...
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      ...
      PresenterTrackImpl track = PresenterTrackImpl.fromFile(
          new File(trackPath), MimeTypes.parseMimeType(mimeType), checksum);
      ...
      throw new BundleException("Error while reading presenter track "
          + trackPath + ": " + e.getMessage());
    }
  }
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException("Error reading presenter track from " + file
          + " : " + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Presenter track " + file
