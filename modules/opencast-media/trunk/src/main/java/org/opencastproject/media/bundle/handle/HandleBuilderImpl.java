...
/**
 * Default implementation of a handle builder. Note that this implementation is
 * for your convenience only, since all it does is creating unique identifiers
 * wrapped into the handle syntax using a globally invalid value of
 * <code>00000</code> for the naming authority and cannot update, resolve or
 * delete these handles.
 * <p>
 ...
 * server, or provide your own solution, e. g. an embedded handle server or the
 * like.
 * </p>
 * 
...
public class HandleBuilderImpl implements HandleBuilder {
  ...
  private final static Logger log_ = LoggerFactory
      .getLogger(HandleBuilderImpl.class);
  ...
  public HandleBuilderImpl() {
    try {
      ...
      log_.error("Unable to create default " + DEFAULT_URL
          + " url for handle builder");
    }
  }
  ...
  public Handle fromValue(String value) throws HandleException {
    if (value == null)
      throw new IllegalArgumentException(
          "Unable to create handle from null string");
  ...
  /**
   * This implementation throws an {@link IllegalArgumentException}, since it
   * cannot resolve handles.
   * 
  ...
  public URL resolve(Handle handle) throws HandleException {
    ...
    throw new IllegalStateException(
        "This implementation cannot resolve handles");
  }
   ...
   * updating anything.
   * 
   * @see org.opencastproject.media.bundle.handle.HandleBuilder#update(org.opencastproject.media.bundle.handle.Handle,
   *      java.net.URL)
   */
