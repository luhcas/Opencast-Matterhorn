...
public class Mpeg7Test {
  ...
  public void setUp() throws Exception {
    ...
    if (!catalogFile.exists() || !catalogFile.canRead())
      throw new Exception("Unable to access mpeg-7 test catalog '"
          + catalogName + "'");
  }

  /**
   * Test method for
   * {@link org.opencastproject.media.bundle.mpeg7.Mpeg7CatalogImpl#fromFile(java.io.File)}
   * .
   */
  @Test
  ...
  /**
   * Test method for
   * {@link org.opencastproject.media.bundle.mpeg7.Mpeg7CatalogImpl#save()}.
   */
  @Test
  ...
  public void testContent(Mpeg7Catalog mpeg7) {
    ...
    // Check content size
    assertTrue(mpeg7.getMultimediaContent(MultimediaContent.Type.AudioType)
        .size() == 1);
    assertTrue(mpeg7.getMultimediaContent(MultimediaContent.Type.VideoType)
        .size() == 2);
    ...
    // Media locator
    assertEquals(track1.getMediaLocator().getMediaURI(), URI
        .create("file:tracks/audio.pcm"));
    // Media time point
    ...
    // Media locator
    assertEquals(track2.getMediaLocator().getMediaURI(), URI
        .create("file:tracks/presentation.mp4"));
    // Media time point
    ...
    assertEquals(v1Decomposition.getCriteria(),
        TemporalDecomposition.DecompositionCriteria.Temporal);
    assertTrue(v1Decomposition.segments().hasNext());
    // Segment track-2.segment-1
    VideoSegment v1Segment1 = v1Decomposition
        .getSegmentById("track-2.segment-1");
    ...
    assertEquals(textAnnotation.keywordAnnotations().next().getKeyword(),
        "Armin");
    assertEquals(textAnnotation.freeTextAnnotations().next().getText(),
        "Hint Armin");
    ...
    // Media locator
    assertEquals(track3.getMediaLocator().getMediaURI(), URI
        .create("file:tracks/presenter.mpg"));
    // Media time point
