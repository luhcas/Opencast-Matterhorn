...
public class BundleMergeTest {
  ...
  private void setUpSourceBundle() throws IOException, BundleException {
    ...
    FileSupport.copy(dcSeriesTestFile, new File(metadataDir,
        "series-dublincore-new.xml"));
    FileSupport.copy(mpeg7TestFile, metadataDir);
    FileSupport.copy(coverTestFile, attachmentDir);
    FileSupport
        .copy(coverTestFile, new File(attachmentDir, "series-cover.png"));
  ...
  private void testBundleConsistency(Bundle bundle) throws BundleException {
    ...
    for (BundleElement e : bundle.elements()) {
      if (ids.contains(e.getIdentifier()))
        throw new BundleException("Duplicate id " + e.getIdentifier()
            + "' found");
      ids.add(e.getIdentifier());
      if (files.contains(e.getFile()))
        throw new BundleException("Duplicate filename " + e.getFile()
            + "' found");
      files.add(e.getFile());
    }
  }
  ...
  public void testMergeByMerging() {
    try {
      Bundle bundle = BundleSupport.merge(targetDir, sourceDir, true,
          MergeMode.Merge);
  ...
  public void testMergeByReplacing() {
    try {
      ...
      // Merge the bundle
      bundle = BundleSupport.merge(targetDir, sourceDir, true,
          MergeMode.Replace);
