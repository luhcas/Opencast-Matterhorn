...
public class NCASTVideoSplitWorkflowOperationHandlerTest {
  ...
  public void setup() throws Exception {
    ...
    EasyMock.expect(
            workspace.get((URI) EasyMock.anyObject())).andReturn(testFile);
    ...
    EasyMock.expect(
            bundleContext.getProperty("org.opencastproject.storage.dir")).andReturn("/tmp");
    EasyMock.replay(bundleContext);
    componentContext = EasyMock.createNiceMock(ComponentContext.class);
    EasyMock.expect(
            componentContext.getProperties()).andReturn(dictionary);
    EasyMock.expect(
            componentContext.getProperties()).andReturn(dictionary);
    EasyMock.expect(
            componentContext.getBundleContext()).andReturn(bundleContext);
  ...
  public void testSplitWithWrongVideoMimeType() throws Exception {
    ...
    URI uriMP = NCASTVideoSplitWorkflowOperationHandler.class.getResource("/split_with_wrong_video_mimetype.xml").toURI();
  ...
  public void testSplitWithDefinedVideo() throws Exception {
      ...
      EasyMock.expect(
              serviceRegistry.getJob(new Long(1))).andReturn(job1);
      EasyMock.expect(
              serviceRegistry.getJob(new Long(2))).andReturn(job2);
