...
/**
 * Class containing audio monitoring services that can be incorporated into GStreamer pipelines and used for confidence
 * monitoring.
 */
public final class AudioMonitoring {
  ...
  /**
   * This constructor should prevent the instantiation of this static utility class.
   */
  private AudioMonitoring() {
    // Nothing to be done her
  }
  ...
  /**
   * Add a method for confidence monitoring to a pipeline capturing audio by teeing the raw data a pulling it to the
   * appsink element.
   * 
   * @param bin
   *          {@code Bin} to add audio monitoring to
   * @param src
   ...
   * @param maxLength
   *          The number of seconds to store RMS data for
   * @param name
   *          The friendly name of the device to add audio monitoring to
   ...
   * @param properties
   *          {@code Properties} that define the interval and maximum time for the confidence monitoring
   */
  public static void getConfidencePipeline(Pipeline pipeline, CaptureDevice c, Properties properties) {
    ...
    final int interval = Integer.parseInt(properties.getProperty(
            CaptureParameters.CAPTURE_DEVICE_PREFIX + c.getFriendlyName()
