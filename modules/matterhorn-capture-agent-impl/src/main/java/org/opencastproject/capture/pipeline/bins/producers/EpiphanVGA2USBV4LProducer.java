...
public class EpiphanVGA2USBV4LProducer extends V4LProducer {
   ...
   * @param captureAgent
   *          The {@code CaptureAgent} that we will use to create the event probe for our source bin that will let us
   *          swap out for our test signal in case the Epiphan card disapears.
   ...
   *           thrown.
   */
  public EpiphanVGA2USBV4LProducer(CaptureDevice captureDevice, Properties properties, CaptureAgent captureAgent)
  ...
  /**
   * Add an event probe to the v4l_identity elements so that we can catch the EOS that propagates when the signal is
   * lost and switch it out for the backup video source
   */
  ...
  protected void addElementsToBin() {
    bin.addMany(v4lsrc, v4lIdentity, queue, videotestsrc, resolutionCapsfilter, staticIdentity, selector, segment,
            videorate, fpsfilter, ffmpegcolorspace);
  }
  ...
  /**
   * Create a capabilities filter that will allow us to set the videotestsrc resolution to the same as the Epiphan
   * capture size. When we swap out the Epiphan card for the videotestsrc they will be the same size.
   * 
   * @throws UnableToCreateElementException
   *           If the capsfilter cannot be created then this Exception will be thrown.
   */
  private Element setupResolutionFilter() throws UnableToCreateElementException {
    Element capsfilter = GStreamerElementFactory.getInstance().createElement(captureDevice.getFriendlyName(),
   ...
   * 
   * @throws UnableToLinkGStreamerElementsException
   *           If any of the Elements cannot be linked together.
   * **/
  @Override
    ...
    public synchronized boolean eventReceived(Pad pad, Event event) {
      ...
      if (event instanceof EOSEvent) {
        ...
        Element src = bin.getElementByName("v4lsrc_" + captureDevice.getLocation() + "_"
                + EpiphanVGA2USBV4LProducer.v4lsrcIndex);
