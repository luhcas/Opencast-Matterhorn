...
public class SchedulerRestService {
  ...
  @Path("{eventID}.xml")
  public Response getEventXml(@PathParam("eventID") String eventId) {
  ...
  @Path("{eventID}.json")
  public Response getEventJson(@PathParam("eventID") String eventId) {
    return getEvent(eventId);
  }
  
  private Response getEvent(String eventId) {
    if(!eventId.isEmpty()){
      try {
  ...
  public Response addEvent (@PathParam("eventId") String eventId, @FormParam("event") EventImpl event) {
    ...
    if(!eventId.isEmpty() && event != null){
      try{
        if(!event.getRecurrencePattern().isEmpty()) { //try to create event and it's recurrences
