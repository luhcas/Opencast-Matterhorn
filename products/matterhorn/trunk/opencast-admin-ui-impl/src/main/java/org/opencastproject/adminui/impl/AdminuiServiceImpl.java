...
/**
 * Backend Service for the Admin App. Provides data mash-up from several
 * services while being tolerant to missing services.
 */
public class AdminuiServiceImpl implements AdminuiService, ManagedService {
  private static final Logger logger = LoggerFactory.getLogger(AdminuiServiceImpl.class);
  ...
  /**
   * returns a RecordingDataViewList of recordings in a certain state.
   * Possible states are: ALL, UPCOMING, PROCESSING, FINISHED
   * @param state state to filter for
   * @return RecordingDataViewList of recordings in a certain state
   */
  public RecordingDataViewList getRecordings(String state) {
