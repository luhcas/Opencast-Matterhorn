    ...
    that.showLinks = function() {
        ...
        $.ajax(
            ...
            success: function(data)
            {
                    ...
                    var buildLinks = function(media){
                        ...
                        for( i = 0; i < media.length; i+=1 ){
                            if ( media[i]["tags"]["tag"].indexOf('engage') === -1 ){
                                ...
                                myHTML += " <a class='button' href="+ media_href + ">" +  media_type + " - " + media_description  + " [ File extension : <em>"+  media_format[media_format.length - 1] +"</em> ] " + "</a><br>";
                            }
                        }
                ...
                $('#oc_client_downloads').append("<div class='download-note'><b>Please Note :</b><br>Some files may not download automatically, you may need to \"Right click\" the download link and select \"Save Link As..\" to save the file. In some browsers where the \"Right click\" is disabled - e.g. Firefox 8 on Mac OS X - you may have to press ALT and then click the download link to prompt a download.</div>");
                ...
                $('#oc_client_downloads').append("<div class='clear'></div>");
