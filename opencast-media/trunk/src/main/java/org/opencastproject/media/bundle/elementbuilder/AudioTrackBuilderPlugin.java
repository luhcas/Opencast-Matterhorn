...
/**
 * This implementation of the {@link BundleElementBuilderPlugin} recognises audio
 * tracks and provides the functionality of reading it on behalf of the bundle.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id$
 */
public class AudioTrackBuilderPlugin extends AbstractTrackBuilderPlugin implements BundleElementBuilderPlugin {
	
	/** the logging facility provided by log4j */
	private final static Logger log_ = LoggerFactory.getLogger(AudioTrackBuilderPlugin.class);
	...
	public boolean accept(BundleElement.Type type, BundleElementFlavor flavor) {
		return type.equals(BundleElement.Type.Track) && flavor.equals(AudioTrack.FLAVOR);
	}
	...
	public boolean accept(File file, BundleElement.Type type, BundleElementFlavor flavor) throws IOException {
		if (type != null && flavor != null) {
			if (!type.equals(BundleElement.Type.Track) || !flavor.equals(AudioTrack.FLAVOR))
				return false;
	...
	public BundleElement elementFromManifest(Node elementNode, File bundleRoot, boolean verify) throws BundleException {
		...
		try {
			...
			AudioTrackImpl track = AudioTrackImpl.fromFile(new File(trackPath), MimeTypes.parseMimeType(mimeType), checksum);
			...
			throw new BundleException("Error while reading audio track " + trackPath + ": " + e.getMessage());
		}
	}
	...
	public BundleElement elementFromFile(File file) throws BundleException {
		try {
			...
			throw new BundleException("Error reading audio track from " + file + " : " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new BundleException("Audio track " + file + " has an unknown file type: " + e.getMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new BundleException("Audio track " + file + " cannot be checksummed: " + e.getMessage());
