 ...
 * @version $Id$
 */
public class PresentationTrackBuilderPlugin extends AbstractTrackBuilderPlugin implements BundleElementBuilderPlugin {
	
	/** The mime type identifier */
	// TODO: Make this configurable
	private static String[] MIME_TYPES = { 
		MimeTypes.MJPEG2000.toString(),
		MimeTypes.MPEG4.toString(),
		MimeTypes.MPEG4_AAC.toString(),
	...
	private final static Logger log_ = LoggerFactory.getLogger(PresentationTrackBuilderPlugin.class);
		...
		if (mimeTypes == null) {
			...
			for (String m : MIME_TYPES) {
				try {
					...
					mimeType.setFlavor(PresentationTrack.FLAVOR.getSubtype(), PresentationTrack.FLAVOR_DESCRIPTION);
					types.add(mimeType);
	...
	public boolean accept(File file, BundleElement.Type type, BundleElementFlavor flavor) throws IOException {
		if (type == null || flavor == null)
			return false;
		else if ((!type.equals(BundleElement.Type.Track) || !flavor.equals(PresentationTrack.FLAVOR)) && !checkFilenamePrefix(file, PresentationTrack.FILENAME_PREFIX))
			return false;
	...
	public BundleElement elementFromManifest(Node elementNode, File bundleRoot, boolean verify) throws BundleException {
		...
		try {
			...
			PresentationTrackImpl track = PresentationTrackImpl.fromFile(new File(trackPath), MimeTypes.parseMimeType(mimeType), checksum);
			...
			throw new ConfigurationException("Unable to calculate checksum for presenation track " + trackPath + ": " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new ConfigurationException("MimeType " + mimeType + " is not supported: " + e.getMessage());
		} catch (IOException e) {
			throw new BundleException("Error while reading presentation track " + trackPath + ": " + e.getMessage());
		}
	}
	...
	public BundleElement elementFromFile(File file) throws BundleException {
		try {
			...
			throw new BundleException("Error reading presentation track from " + file + " : " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new BundleException("Presentation track " + file + " has an unknown file type: " + e.getMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new BundleException("Presentation track " + file + " cannot be checksummed: " + e.getMessage());
