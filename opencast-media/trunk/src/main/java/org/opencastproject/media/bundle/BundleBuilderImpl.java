...
/**
 * This class provides factory methods for the creation of bundles from manifest
 * files, directories or from sratch.
 * 
...
public class BundleBuilderImpl implements BundleBuilder {
  ...
  /**
   * Creates a new bundle builder. The builder will try to setup a temporary
   * directory from the java environment and throws Exceptions if this operation
   * fails.
   * 
  ...
  public Bundle createNew(Handle identifier) throws BundleException {
    String bundleRootPath = PathSupport.concat(new String[] {
        TMP_DIR.getAbsolutePath(), identifier.getNamingAuthority(),
   ...
   * @see org.opencastproject.media.bundle.BundleBuilder#createNew(org.opencastproject.media.bundle.handle.Handle)
   */
  public Bundle createNew(Handle identifier, File bundleRoot)
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleBuilder#loadFromManifest(java.io.File,
   *      boolean)
   */
  public Bundle loadFromManifest(File manifestFile, boolean wrap)
      throws BundleException {
    ...
    if (!manifestFile.getName().equals(Manifest.FILENAME))
      throw new BundleException("Manifest file must be named "
          + Manifest.FILENAME);
    ...
    try {
      ...
      throw new BundleException("I/O error while accessing manifest: "
          + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Bundle manifest is of unsuported mime type: "
          + e.getMessage());
    } catch (ParserConfigurationException e) {
      throw new BundleException("Bundle manifest cannot be parsed: "
          + e.getMessage());
    } catch (TransformerException e) {
      throw new BundleException("Error while updating bundle manifest: "
          + e.getMessage());
    } catch (SAXException e) {
      throw new BundleException("Error while parsing bundle manifest: "
          + e.getMessage());
      ...
      throw new BundleException(
          "Configuration error while accessing manifest: " + e.getMessage());
    } catch (HandleException e) {
      throw new BundleException(
          "Handle system error while accessing manifest: " + e.getMessage());
    } catch (ParseException e) {
      throw new BundleException(
          "Error while parsing invalid bundle start date: " + e.getMessage());
    }
  ...
  public Bundle loadFromRepository(String path) throws BundleException {
    ...
    // e.getMessage());
    // } catch (IOException e) {
    // throw new BundleException("IO Exception while accessing manifest: " +
    // e.getMessage());
    // } catch (UnknownFileTypeException e) {
    // throw new BundleException("Bundle manifest is of unsuported mime type: "
    // + e.getMessage());
    ...
    // e.getMessage());
    // } catch (ParseException e) {
    // throw new BundleException("Error while parsing bundle start date: " +
    // e.getMessage());
    // }
    //    
    // return new BundleImpl(manifest);
   ...
   *      boolean)
   */
  public Bundle rebuildFromDirectory(File dir, boolean ignoreChecksums)
      throws BundleException {
  ...
  public Bundle rebuildFromDirectory(File dir, boolean ignoreChecksums,
    ...
    // Read the manifest
    try {
      manifest = ManifestImpl.fromFile(manifestFile, true, ignoreChecksums,
          verify);
    } catch (NoSuchAlgorithmException e) {
      throw new BundleException("Unsupported digest algorithm: "
          + e.getMessage());
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleBuilder#createFromElements(File,
   *      boolean)
   */
  public Bundle createFromElements(File dir, boolean ignoreUnknown)
    ...
    try {
      ...
      throw new BundleException("Unable to retreive new handle: "
          + e.getMessage());
    }
    ...
    try {
      ...
      BundleElementBuilderFactory elementBuilderFactory = BundleElementBuilderFactory
          .newInstance();
      BundleElementBuilder elementBuilder = elementBuilderFactory
          .newElementBuilder();
      ...
      throw new BundleException("I/O error while reading bundle elements: "
          + e.getMessage());
    } catch (UnknownFileTypeException e) {
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleBuilder#loadFromPackage(org.opencastproject.media.bundle.BundlePackager,
   *      java.io.InputStream)
   */
  public Bundle loadFromPackage(BundlePackager packager, InputStream in)
  ...
  /**
   * Creates a new bundle by creating a directory within <code>TMP_DIR</code>
   * and an emtpy manifest file.
   * 
   * @param bundleRoot
   *          the base dir
   * @param identifier
   ...
   */
  private File createBundleFilesystem(File bundleRoot, Handle identifier)
      throws BundleException {
    ...
    try {
      ...
      if (!bundleRoot.exists())
        bundleRoot.mkdirs();
      if (!bundleRoot.exists())
        throw new BundleException("Unable to create bundle directory "
            + bundleRoot.getAbsolutePath());
      // Always create a new manifest
      if (manifestFile.exists() && !manifestFile.delete()) {
        throw new BundleException(
            "Unable to delete existing manifest on bundle filesystem recreation");
      }
      manifestFile.createNewFile();
      ManifestImpl.newInstance(bundleRoot, identifier);
      return bundleRoot;
    } catch (IOException e) {
      throw new BundleException("Unable to create bundle " + identifier + ": "
          + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Unable to create bundle " + identifier + ": "
          + e.getMessage());
      ...
      throw new BundleException("Error creating bundle " + identifier + ": "
          + e.getMessage());
    }
  }
