...
public final class BundleImpl implements Bundle {
	...
	/**
	 * Creates a bundle object from the given root directory and bundle identifier.
	 *
	 * @param directory the bundle root directory
	 * @param handle the bundle identifier
	 * @throws IOException
	 ...
	 */
	BundleImpl(File directory, Handle handle) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
		this.rootDir = directory;
		this.manifest = new ManifestImpl(this, handle);
	}

	/**
	 * Creates a new media bundle derived from the given media bundle
	 * catalog document.
	 *
	 ...
	 * @return <code>true</code> if the bundle contains an element with this identifier
	 */
	 ...
	 */
	public void add(Catalog catalog) throws BundleException, UnsupportedBundleElementException {
        add(catalog, true);
	}

    public void add(Catalog catalog, boolean move) throws BundleException, UnsupportedBundleElementException {
        try {
            integrateCatalog(catalog, move);
            manifest.add(catalog);
            fireElementAdded(catalog);
        } catch (IOException e) {
            throw new BundleException("Error integrating " + catalog + " into bundle: " + e.getMessage());
        }
    }
	 ...
	 */
	public void add(Track track) throws BundleException, UnsupportedBundleElementException {
        add(track, true);
	}

    /**
     * @see org.opencastproject.media.bundle.Bundle#add(org.opencastproject.media.bundle.Track, boolean)
     */
	public void add(Track track, boolean move) throws BundleException, UnsupportedBundleElementException {
		try {
			integrateTrack(track, move);
			manifest.add(track);
			fireElementAdded(track);
		} catch (IOException e) {
			throw new BundleException("Error integrating " + track + " into bundle: " + e.getMessage());
		}
	}
	 ...
	 */
	public void add(Attachment attachment) throws BundleException, UnsupportedBundleElementException {
        add(attachment, true);
    }

    public void add(Attachment attachment, boolean move) throws BundleException, UnsupportedBundleElementException {
        try {
            integrateAttachment(attachment, move);
            manifest.add(attachment);
            fireElementAdded(attachment);
        } catch (IOException e) {
            throw new BundleException("Error integrating " + attachment + " into bundle: " + e.getMessage());
        }
    }
	...
	/**
	 * @see org.opencastproject.media.bundle.Bundle#getCatalogs(org.opencastproject.media.bundle.BundleElementFlavor, org.opencastproject.media.bundle.BundleReference)
	 */
	public Catalog[] getCatalogs(BundleElementFlavor flavor, BundleReference reference) {
	...
	/**
	 * @see org.opencastproject.media.bundle.Bundle#getTracks(org.opencastproject.media.bundle.BundleElementFlavor, org.opencastproject.media.bundle.BundleReference)
	 */
	...
	/**
	 * @see org.opencastproject.media.bundle.Bundle#getAttachments(org.opencastproject.media.bundle.BundleElementFlavor, org.opencastproject.media.bundle.BundleReference)
	 */
	public Attachment[] getAttachments(BundleElementFlavor flavor, BundleReference reference) {
	...
	public Cover getCover() {
		...
		if (covers.length > 0) {
			if (covers[0] instanceof Cover)
				...
				log_.warn("Cover with inconsistant object type contained in bundle " + this);
				return null;
			}
		}
		return null;
	 ...
	 */
	public void setCover(Cover cover) throws BundleException, UnsupportedBundleElementException {
		setCover(cover, true);
	}

	/**
	 * @see org.opencastproject.media.bundle.Bundle#setCover(org.opencastproject.media.bundle.Cover, boolean)
	 */
	public void setCover(Cover cover, boolean move) throws BundleException, UnsupportedBundleElementException {
	...
	/**
	 * @see org.opencastproject.media.bundle.Bundle#pack(org.opencastproject.media.bundle.BundlePackager, java.io.OutputStream)
	 */
	public void pack(BundlePackager packager, OutputStream out) throws IOException, BundleException {
		if (packager == null)
	 ...
	 */
	public BundleElement add(File file) throws BundleException, UnsupportedBundleElementException {
	 ...
	 */
	public BundleElement add(File file, boolean move) throws BundleException, UnsupportedBundleElementException {
	 ...
	 */
	public void add(BundleElement element) throws BundleException, UnsupportedBundleElementException {
        add(element, true);
	}

    public void add(BundleElement element, boolean move) throws BundleException, UnsupportedBundleElementException {
        try {
            if (element.getElementType().equals(BundleElement.Type.Track) && element instanceof Track) {
                integrateTrack((Track)element, move);
            } else if (element.getElementType().equals(BundleElement.Type.Catalog) && element instanceof Catalog) {
                integrateCatalog((Catalog)element, move);
            } else if (element.getElementType().equals(BundleElement.Type.Attachment) && element instanceof Attachment) {
            ...
            throw new BundleException("Error integrating " + element + " into bundle: " + e.getMessage());
        }
    }
		...
		synchronized (observers) {
			for (BundleObserver o : observers) {
                try {
                    ...
                    log_.error("BundleOberserver " + o + " throw exception while processing callback", th);
                }
            }
		}
	}
		...
		synchronized (observers) {
			for (BundleObserver o : observers) {
                try {
                    ...
                    log_.error("BundleObserver " + o + " threw exception while processing callback", th);
                }
            }
		}
	}
	...
	/**
	 * Integrates the element into the bundle. This mainly involves moving
	 * the element into the bundle file structure.
	 *
	...
	private void integrate(BundleElement element) throws IOException {
		...
		throw new IllegalStateException("This element is not supported by this bundle implementation");
	}

	/**
	 * Integrates the catalog into the bundle. This mainly involves moving
	 * the catalog into the bundle file structure.
	 *
	 * @param catalog the catalog to integrate
	 * @param move
     * @throws IOException
    ...
    /**
	 * Integrates the track into the bundle. This mainly involves moving
	 * the track into the bundle file structure.
	 *
	 * @param track the track to integrate
	 * @param move
     * @throws IOException
	...
	/**
	 * Integrates the attachment into the bundle. This mainly involves moving
	 * the attachment into the bundle file structure.
	 *
	 * @param attachment the attachment to integrate
	 * @param move
     * @throws IOException
    ...
    /**
     * Integrates (= copies) the elements underlying resource into the bundle and wires the
     * element with the bundle
     *
     * @param element the element to integrate
     * @param elementRoot the directory, where the element shall reside within the bundle
     * @throws IOException
     */
    private void integrateIntoBundle(BundleElement element, File elementRoot) throws IOException {
	 ...
	 * number or a higher one as the suffix. The identifier will be unique
	 * within the bundle.
	 *
