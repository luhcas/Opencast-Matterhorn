...
public class VideoTrackImpl extends TrackImpl implements VideoTrack {
	 ...
	 */
	protected VideoTrackImpl(BundleElementFlavor flavor, File track) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
		super(flavor, track);
	}
	 ...
	 * 		if the md5 checksum cannot be computed
	 */
	protected VideoTrackImpl(BundleElementFlavor flavor, MimeType mimeType, File track, Checksum checksum) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
	 ...
	 * 		if the md5 checksum cannot be computed
	 */
	public static VideoTrackImpl fromFile(File file) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
	 ...
	 * 		if the md5 checksum cannot be computed
	 */
	public static VideoTrackImpl fromFile(File file, MimeType mimeType, Checksum checksum) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
	...
	public String toString() {
		...
		if (hasAudio() && getAudioSettings().getMetadata().getFormat() != null) {
			...
			if (!hasVideo() || !audioCodec.equals(getVideoSettings().getMetadata().getFormat())) {
				if (details)
					buf.append("/");
				details = true;
