 ...
 * audiovisual tracks and provides the functionality of reading it on behalf of
 * the bundle.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id: AudioVisualTrackBuilderPlugin.java 102 2009-04-03 20:05:36Z
 *          wunden $
 */
public class AudioVisualTrackBuilderPlugin extends AbstractTrackBuilderPlugin
    implements BundleElementBuilderPlugin {

  /** the logging facility provided by log4j */
  private final static Logger log_ = LoggerFactory
      .getLogger(AudioVisualTrackBuilderPlugin.class);
  ...
  public boolean accept(File file, BundleElement.Type type,
    ...
    if (type != null && flavor != null) {
      if (!type.equals(BundleElement.Type.Track)
          || !flavor.equals(AudioVisualTrack.FLAVOR))
        return false;
    ...
    // Finally, see if we can analyze the track file and find a audiovisual
    // track
  ...
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      ...
      AudioVisualTrackImpl track = AudioVisualTrackImpl.fromFile(new File(
      ...
      // audio settings
      Node audioSettings = (Node) xpath.evaluate("Audio", elementNode,
          XPathConstants.NODE);
      ...
      throw new BundleException("Error while reading audiovisual track "
          + trackPath + ": " + e.getMessage());
    }
  }
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException("Error reading audiovisual track from " + file
          + " : " + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Audiovisual track " + file
