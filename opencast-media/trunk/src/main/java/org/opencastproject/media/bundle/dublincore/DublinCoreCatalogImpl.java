 ...
 * @version $Id$
 */
public class DublinCoreCatalogImpl extends XMLCatalog
        implements DublinCoreCatalog {
     ...
     */
    static final Logger log_ = LoggerFactory.getLogger(DublinCoreCatalogImpl.class);
     ...
     * namespace is the default namespace of XML documents generated by this class.
     */
    public static final String REPLAY_DUBLIN_CORE_NS_URI = "http://www.replay.ethz.ch/xsd/1.0/dublincore/";

    public static final EName PROPERTY_PROMOTED = new EName(REPLAY_NS_URI, "promoted");
    public static final EName PROPERTY_ADVERTISED = new EName(REPLAY_NS_URI, "advertised");

    static final EName ROOT_ELEMENT = new EName(REPLAY_DUBLIN_CORE_NS_URI, "metadata");
    ...
    DublinCoreCatalogImpl(File file) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
        ...
        // Namespace bindings
        bindings.bindPrefix(XMLConstants.DEFAULT_NS_PREFIX, REPLAY_DUBLIN_CORE_NS_URI);
    ...
    /**
     * Creates a new dublin core metadata container file at the specified location for a bundle
     * with the given identifier.
     *
     * @return the new dublin core metadata container
     * @throws UnknownFileTypeException if the dublin core file type is unknown (very unlikely)
     * @throws IOException if creating the dublin core file fails
     * @throws TransformerException if saving the dublin core file fails
     * @throws ParserConfigurationException if creating the xml parser fails
     * @throws NoSuchAlgorithmException if the md5 checksum cannot be computed
     */
    static DublinCoreCatalog newInstance(Bundle bundle) throws IOException, UnknownFileTypeException, ParserConfigurationException, TransformerException, NoSuchAlgorithmException {
        DublinCoreCatalogImpl dc = new DublinCoreCatalogImpl(new File(bundle.getRoot(), DublinCoreCatalog.FILENAME));
        return dc;
    }
     ...
     * @throws UnknownFileTypeException if the dublin core file is of an unknown file type
     * @throws ParserConfigurationException if the dublin core parser cannot be created
     * @throws NoSuchAlgorithmException if the md5 checksum cannot be computed
     * @throws SAXException if reading the catalog fails
     */
    public static DublinCoreCatalog fromFile(File catalog) throws IOException, UnknownFileTypeException, ParserConfigurationException, NoSuchAlgorithmException, SAXException {
        DublinCoreParser parser = new DublinCoreParser();
        return parser.parse(catalog);
    }

    /**
     * Returns a text representation of the dublin core content. It is intended
     * to be used to easily search the catalog.
     *
    ...
    public List<String> get(EName property, final String language) {
        if (property == null) throw new IllegalArgumentException("Property name must not be null");
        if (language == null) throw new IllegalArgumentException("Language code must not be null");

        if (LANGUAGE_ANY.equals(language)) {
            return cast(CollectionUtils.collect(getValuesAsList(property), new Transformer() {
            ...
            CollectionUtils.forAllDo(getValuesAsList(property), new Closure() {
                public void execute(Object o) {
                    ...
                    if ((langUndef && lang == null) || (language.equals(lang))) values.add(lang);
                }
            });
            return values;
        }
    ...
    public List<DublinCoreValue> get(EName property) {
        ...
        return cast(CollectionUtils.collect(getValuesAsList(property), new org.apache.commons.collections.Transformer() {
            public Object transform(Object o) {
    ...
    private String _getFirst(EName property, String language) {
        ...
        if (LANGUAGE_UNDEFINED.equals(language)) {
            ...
            for (CatalogEntry value : getValuesAsList(property)) {
                ...
                if (!value.hasAttribute(XML_LANG_ATTR)) break;
            }
        } else {
    ...
    public String getAsText(EName property, String language, String delimiter) {
        ...
        return values.size() > 0
                ? StringUtils.join(CollectionUtils.collect(values, new Transformer() {
            public Object transform(Object o) {
                return ((CatalogEntry) o).getValue();
            }
        }), delimiter)
    ...
    public Set<String> getLanguages(EName property) {
        ...
        for (CatalogEntry entry : getValuesAsList(property)) {
            ...
            else languages.add(LANGUAGE_UNDEFINED);
        }
        return languages;
    }
    ...
    private boolean _hasMultipleValues(EName property, String language) {
        if (LANGUAGE_ANY.equals(language)) {
            ...
            int counter = 0;
            for (CatalogEntry entry : getValuesAsList(property)) {
                if (equalLanguage(language, entry.getAttribute(XML_LANG_ATTR))) counter++;
    ...
    public void set(EName property, DublinCoreValue value) {
        ...
        if (value != null) {
            _set(property, value.getValue(), value.getLanguage(), value.getEncodingScheme());
        } else {
    ...
    private void _set(EName property, String value, String language, EName encodingScheme) {
        if (value == null) {
            ...
            String lang = !LANGUAGE_UNDEFINED.equals(language)
                    ? language : null;
    ...
    public void add(EName property, DublinCoreValue value) {
        ...
        _add(property, value.getValue(), value.getLanguage(), value.getEncodingScheme());
    }

    private void _add(EName property, String value, String language, EName encodingScheme) {
        if (LANGUAGE_UNDEFINED.equals(language)) {
     ...
     * @throws TransformerException if serialization of the metadata document fails
     * @throws IOException if an error with catalog file handling occurs
     */
    ...
    /**
     * Adds the dublin core property identified by <code>name</code> to the
     * root node if it's value is different from <code>null</code>.
     *
     * @param document the document
     * @param root the root node
     * @param property the property name
     * @param required <code>true</code> if this property is required
     * @return <code>true</code> if the element was added
     * @throws IllegalStateException if a required property is missing
     */
    private boolean addElement(Document document, Element root, EName property, boolean required) throws IllegalStateException {
    ...
    static class DublinCoreParser extends DefaultHandler {
         ...
         *
         * @param file the file containing the dublin core catalog
         * @return the catalog representation
         * @throws javax.xml.parsers.ParserConfigurationException if setting up the parser failed
         * @throws org.xml.sax.SAXException if an error occured while parsing the document
         * @throws java.io.IOException if the file cannot be accessed in a proper way
         * @throws ch.ethz.replay.core.api.exception.UnknownFileTypeException if the catalog file type is unknown
         * @throws java.security.NoSuchAlgorithmException if the checksum cannot be calculated
         * @throws IllegalArgumentException if the
         */
        public DublinCoreCatalogImpl parse(File file) throws SAXException, IOException, ParserConfigurationException, NoSuchAlgorithmException, UnknownFileTypeException {
