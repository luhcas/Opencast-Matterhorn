...
/**
 * Default implementation for video settings as found in the bundle manifest
...
public class VideoSettingsImpl implements VideoSettings, Cloneable {
     ...
     */
    public static VideoSettings fromManifest(Node node, XPath xpath) throws IllegalStateException, XPathException {
    ...
    public Node toManifest(Document document) {
        ...
        if (metadata.getScanType() != null) {
            ...
            if (metadata.getScanOrder() != null)
                interlacingNode.setAttribute("order", metadata.getScanOrder().toString());
        ...
        if (metadata.getFrameRate() != null) {
            ...
            framerateNode.appendChild(document.createTextNode(metadata.getFrameRate().toString()));
