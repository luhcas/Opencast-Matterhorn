...
/**
 * This general inplementation of a bundle element builder supports specialized
 * implementations by providing tests on the filename an mime type of the file
 * in question.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id: AbstractElementBuilderPlugin.java 102 2009-04-03 20:05:36Z
 *          wunden $
 */
public abstract class AbstractElementBuilderPlugin implements
    BundleElementBuilderPlugin {
  ...
  private final static Logger log_ = LoggerFactory
      .getLogger(AbstractElementBuilderPlugin.class);
   ...
   * 
   * @see org.opencastproject.media.bundle.BundleElementBuilderPlugin#setup()
   */
   ...
   * This is a convenience implementation for subclasses doing nothing.
   * 
   * @see org.opencastproject.media.bundle.BundleElementBuilderPlugin#cleanup()
   */
  public void cleanup() {
  }
  ...
  /**
   * Registers the given mime types with the bundle element builder.
   * 
   * @param mimeType
   ...
   */
  protected void registerMimeType(String mimeType, String description)
      throws UnknownFileTypeException {
    ...
    log_.debug("Processing of documents of type " + description + "(" + m
        + ") enabled");
  }

  /**
   * Returns <code>true</code> if the file's mime type could be detected, is
   * known and supported.
   * 
   * @param file
   *          the file
   ...
   */
  protected boolean checkMimeType(File file, MimeType[] mimeTypes)
   ...
   *           if the file's mime type cannot be gathered or is unknown
   */
  protected boolean checkMimeType(File file) throws IOException,
   ...
   *           if the file cannot be accessed
   */
  protected boolean checkFilename(File file, String filename)
      throws IOException {
    if (file == null)
      throw new IllegalArgumentException("File is null");
    else if (!file.isFile())
  ...
  /**
   * Returns <code>true</code> if the file's name starts with
   * <code>prefix</code>.
   * 
   * @param file
   *          the file
   ...
   *           if the file cannot be accessed
   */
  protected boolean checkFilenamePrefix(File file, String prefix)
      throws IOException {
    if (file == null)
      throw new IllegalArgumentException("File is null");
    else if (!file.isFile())
  ...
  /**
   * Checks the file for its existence and validates it's checksum with the
   * proviced values.
   * 
   * @param file
   *          the file to check
   ...
   */
  protected boolean verifyFileIntegrity(File file, Checksum checksum)
