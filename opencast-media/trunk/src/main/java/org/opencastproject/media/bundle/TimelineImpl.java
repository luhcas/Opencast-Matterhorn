...
public class TimelineImpl extends XMLCatalog implements Timeline {
	...
	/**
	 * Creates a new timeline file at the specified location for a bundle with the given
	 * identifier.
	 * 
	 ...
	 * 		if the md5 checksum cannot be computed
	 */
	static TimelineImpl newInstance(Bundle bundle) throws IOException, UnknownFileTypeException, ParserConfigurationException, TransformerException, NoSuchAlgorithmException {
		TimelineImpl timeline = new TimelineImpl(new File(bundle.getRoot(), Timeline.FILENAME));
		timeline.save();
		return timeline;
	}
	 ...
	 */
	public static TimelineImpl fromFile(File file) throws IOException, UnknownFileTypeException, ParserConfigurationException, NoSuchAlgorithmException, SAXException {
	...
	static class TimelineSAXHandler extends DefaultHandler {
		 ...
		 * 
		 * @param timeline the timeline document
		 */
