 ...
 * @version $Id$
 */
public class MPEG7BuilderPlugin extends AbstractElementBuilderPlugin implements
  ...
  private final static Logger log_ = LoggerFactory
      .getLogger(MPEG7BuilderPlugin.class);
    ...
    if (mimeTypes == null) {
      ...
      for (String m : MIME_TYPES) {
        try {
          ...
          mimeType.setFlavor(Mpeg7Catalog.FLAVOR.getSubtype(),
              FLAVOR_DESCRIPTION);
          types.add(mimeType);
          log_.debug("Building of mpeg-7 catalogs enabled");
        } catch (Exception e) {
          log_.warn("Unable to create mpeg7 documents: mimetype " + m
              + " is not supported");
          throw e;
        }
      }
  ...
  public boolean accept(File file, BundleElement.Type type,
    ...
    try {
      ...
      if (type != null && flavor != null)
        return type.equals(org.opencastproject.media.bundle.Catalog.TYPE)
            && flavor.equals(Mpeg7Catalog.FLAVOR);
      else if (type != null
          && !type.equals(org.opencastproject.media.bundle.Catalog.TYPE))
        return false;
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException("Error reading mpeg-7 from " + file + " : "
          + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("MPEG-7 metadata " + file
          + " has an unknown file type: " + e.getMessage());
    } catch (ParserConfigurationException e) {
      throw new BundleException(
          "Parser configuration exception while reading mpeg-7 metadata from "
              + file + " : " + e.getMessage());
    } catch (NoSuchAlgorithmException e) {
      throw new BundleException("MPEG-7 metadata " + file
          + " cannot be checksummed: " + e.getMessage());
    } catch (SAXException e) {
      throw new BundleException("Error parsing mpeg-7 catalog " + file + " : "
          + e.getMessage());
    }
  }
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleElementBuilder#elementFromManifest(org.w3c.dom.Node,
   *      File, boolean)
   */
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      ...
      throw new BundleException("Unable to create parser for mpeg-7 catalog "
          + catalogPath + ": " + e.getMessage());
    } catch (IOException e) {
      throw new BundleException("Error while reading mpeg-7 file "
          + catalogPath + ": " + e.getMessage());
    } catch (SAXException e) {
      throw new BundleException("Error while parsing mpeg-7 catalog "
          + catalogPath + ": " + e.getMessage());
    }
  }
