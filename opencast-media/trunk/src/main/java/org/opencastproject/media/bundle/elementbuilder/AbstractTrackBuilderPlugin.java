...
public abstract class AbstractTrackBuilderPlugin extends AbstractElementBuilderPlugin {
	...
	private final static Logger log_ = LoggerFactory.getLogger(AbstractTrackBuilderPlugin.class);
	...
	/**
	 * Returns the {@link ch.ethz.replay.core.api.common.media.analysis.MediaContainerMetadata}
	...
	/**
	 * Adds the media info extracted by the {@link ch.ethz.replay.core.api.common.media.analysis.MediaAnalyzer}.
	 */
	protected void addMediaInfo(TrackImpl track) throws MediaAnalyzerException {
        ...
        if (mediaInfo.hasVideoStreamMetadata()) {
            ...
            if (metadata.size() > 1)
                log_.warn("Analysis reveals that " + track.getFile().getAbsolutePath() + " contains " +
                        metadata.size() + " video streams. Because the VideoSettings " +
		...
		if (mediaInfo.hasAudioStreamMetadata()) {
            ...
            if (metadata.size() > 1)
                log_.warn("Analysis reveals that " + track.getFile().getAbsolutePath() + " contains " +
                        metadata.size() + " audio streams. Because the AudioSettings " +
                        "can handle only one stream, only the first one will be used.");
