 ...
 * dublin core file format and provides the functionality of reading it on
 ...
 * @version $Id$
 */
public class DublinCoreBuilderPlugin extends AbstractElementBuilderPlugin
  ...
  private final static Logger log_ = LoggerFactory
      .getLogger(DublinCoreBuilderPlugin.class);
    ...
    if (mimeTypes == null) {
      ...
      for (String m : MIME_TYPES) {
        try {
          ...
          mimeType.setFlavor(DublinCoreCatalog.FLAVOR.getSubtype(),
              FLAVOR_DESCRIPTION);
          types.add(mimeType);
          log_.debug("Building of dublin core catalogs enabled");
        } catch (Exception e) {
          log_.warn("Unable to create dublin core documents: mimetype " + m
              + " is not supported");
        }
      }
  ...
  public boolean accept(File file, BundleElement.Type type,
    ...
    try {
      ...
      if (type != null && flavor != null)
        return type.equals(Catalog.TYPE)
            && flavor.equals(DublinCoreCatalog.FLAVOR);
  ...
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      ...
      // create the catalog
      DublinCoreCatalog dc = DublinCoreCatalogImpl.fromFile(new File(
          catalogPath));
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException(
          "Parser configuration exception while reading dublin core catalog from "
              + file + " : " + e.getMessage());
    } catch (NoSuchAlgorithmException e) {
      throw new BundleException("Dublin core document " + file
          + " cannot be checksummed: " + e.getMessage());
    } catch (SAXException e) {
      throw new BundleException("Error parsing dublin core catalog " + file
          + " : " + e.getMessage());
    }
  }
