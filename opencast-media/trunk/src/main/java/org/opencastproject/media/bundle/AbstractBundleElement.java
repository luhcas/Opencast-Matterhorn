 ...
 * @version $Id$
 */
public abstract class AbstractBundleElement implements BundleElement, Serializable {
	 ...
	 * 
	 * @param elementType the type, e. g. Track, Catalog etc.
	 ...
	 */
	protected AbstractBundleElement(Type elementType, BundleElementFlavor flavor, File file) throws IOException, UnknownFileTypeException, NoSuchAlgorithmException {
	 ...
	 * 		if the file type is unknown
	 */
	protected AbstractBundleElement(Type elementType, BundleElementFlavor flavor, File file, Checksum checksum) throws IOException, UnknownFileTypeException {
	 ...
	 * 		if the md5 checksum cannot be computed
	 */
	protected AbstractBundleElement(Type elementType, BundleElementFlavor flavor, MimeType mimeType, File file) throws IOException, NoSuchAlgorithmException {
	 ...
	 * 		if the file cannot be accessed
	 */
	protected AbstractBundleElement(Type elementType, BundleElementFlavor flavor, MimeType mimeType, File file, Checksum checksum) throws IOException {
	 ...
	 * 		if the file cannot be accessed
	 */
	protected AbstractBundleElement(String id, Type elementType, BundleElementFlavor flavor, MimeType mimeType, File file, Checksum checksum) throws IOException {
		if (elementType == null)
	...
	/**
	 * @see org.opencastproject.media.bundle.BundleElement#bundleMoved(File, java.io.File)
	 */
	 ...
	 * <p>
	 * <b>Note</b> This method is only used by the bundle and should not be
	...
	private Checksum calculateChecksum() throws BundleException {
		...
		if (checksum != null) {
			try {
				...
				throw new BundleException("Unable to compute checksum for " + this + ": Checksum algorithm for " + checksum.getType() + " not found: " + e);
			} catch (Throwable t) {
				throw new BundleException("Unable to compute checksum for " + this + ": " + t.getMessage());
			}
		}
		return c;
	}
	...
	public Node toManifest(Document document) {
		...
		if (reference != null && (bundle == null || !reference.matches(new BundleReferenceImpl(bundle))))
