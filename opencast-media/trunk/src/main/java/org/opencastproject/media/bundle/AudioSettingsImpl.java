...
/**
 * Default implementation for audio settings as found in the bundle manifest as
...
public class AudioSettingsImpl implements AudioSettings, Cloneable {
   ...
   */
  public static AudioSettings fromManifest(Node node, XPath xpath)
    ...
    // bit depth
    try {
      String bd = (String) xpath.evaluate("BitDepth/text()", node,
          XPathConstants.STRING);
      if (!StringSupport.isEmpty(bd))
        metadata.setResolution(new Integer(bd.trim()));
    } catch (NumberFormatException e) {
      throw new IllegalStateException("Bit depth was malformatted: "
          + e.getMessage());
    }

    // channels
    try {
      String strChannels = (String) xpath.evaluate("Channels/text()", node,
          XPathConstants.STRING);
      if (!StringSupport.isEmpty(strChannels))
        metadata.setChannels(new Integer(strChannels.trim()));
    } catch (NumberFormatException e) {
      throw new IllegalStateException("Number of channels was malformatted: "
          + e.getMessage());
    }

    // sampling rate
    try {
      String sr = (String) xpath.evaluate("FrameRate/text()", node,
          XPathConstants.STRING);
    ...
    // Bit rate
    try {
      String br = (String) xpath.evaluate("BitRate/text()", node,
          XPathConstants.STRING);
    ...
    // device
    String captureDevice = (String) xpath.evaluate("Device/@type", node,
        XPathConstants.STRING);
    if (!StringSupport.isEmpty(captureDevice))
      metadata.setCaptureDevice(captureDevice);
    String captureDeviceVersion = (String) xpath.evaluate("Device/@version",
    ...
    String captureDeviceVendor = (String) xpath.evaluate("Device/@vendor",
        node, XPathConstants.STRING);
    if (!StringSupport.isEmpty(captureDeviceVendor))
      metadata.setCaptureDeviceVendor(captureDeviceVendor);

    // encoder
    String format = (String) xpath.evaluate("Encoder/@type", node,
        XPathConstants.STRING);
    if (!StringSupport.isEmpty(format))
      metadata.setFormat(format);
    String formatVersion = (String) xpath.evaluate("Encoder/@version", node,
        XPathConstants.STRING);
    if (!StringSupport.isEmpty(formatVersion))
      metadata.setFormatVersion(formatVersion);
    String encoderLibraryVendor = (String) xpath.evaluate("Encoder/@vendor",
        node, XPathConstants.STRING);
  ...
  public Node toManifest(Document document) {
    ...
    if (metadata.getChannels() != null) {
      ...
      channelsNode.appendChild(document.createTextNode(metadata.getChannels()
          .toString()));
    ...
    if (metadata.getResolution() != null) {
      ...
      bitdepthNode.appendChild(document.createTextNode(metadata.getResolution()
          .toString()));
    ...
    if (metadata.getSamplingRate() != null) {
      ...
      samplingrateNode.appendChild(document.createTextNode(metadata
          .getSamplingRate().toString()));
