...
public interface HandleBuilder {
	...
	/**
	 * Creates a new handle by connecting to the handle server and asking it to
	 * hand over a new instance for the default url. Do not forget to update
	 * the handle once the final url is known.
	 * 
	...
	/**
	 * Creates a new handle by connecting to the handle server and asking it to
	 * hand over a new instance for the specified url.
	 * 
	 * @param url the handle target
	 * @return the new handle
	 * @throws HandleException
	 * 		if the handle cannot be created
	 */
	...
	/**
	 * Creates a handle object from the specified value. The value must be a
	 * valid handle in string form, e. g. <code>10.254/test</code>, optionally
	 * preceeded by the handle protocol identifier {@link Handle#PROTOCOL};
	 * 
	 ...
	 * <p>
	 * Note that removing a handle should not occur for urls that have been in
	 * handed out. Rather update the handle to some meaningful <code>404</code>
	 * page instead.
	 * </p>
	 * 
	 * @param handle the handle
