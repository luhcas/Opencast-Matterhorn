 ...
 * href="http://dublincore.org/documents/dcmi-period/"
...
public class DCMIPeriod {
  ...
  public DCMIPeriod(Date start, Date end) {
    Assert.not(start == null && end == null,
        "A period must be bounded at least at one end");
    Assert.not(start != null && end != null && end.before(start),
  ...
  /**
   * Create a new period with an optional name. To create an open interval you
   * may set one of the bounbaries null.
   */
  ...
  /**
   * Returns the start date of the period or null, if it has only an upper
   * bound.
   */
  ...
  public String toString() {
    return "DCMIPeriod{" + "start=" + (start != null ? start : "]") + ", end="
        + (end != null ? end : "[")
        + (name != null ? ", name='" + name + '\'' : "") + '}';
  }
}
