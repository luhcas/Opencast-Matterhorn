...
import org.opencastproject.media.bundle.PresenterTrackImpl;
...
/**
 * This implementation of the {@link BundleElementBuilderPlugin} recognises movie
 ...
 * @version $Id$
 */
public class PresenterTrackBuilderPlugin extends AbstractTrackBuilderPlugin implements BundleElementBuilderPlugin {
	
	/** the logging facility provided by log4j */
	private final static Logger log_ = LoggerFactory.getLogger(PresenterTrackBuilderPlugin.class);
	...
	public boolean accept(Type type, BundleElementFlavor flavor) {
		return type.equals(Type.Track) && flavor.equals(PresenterTrack.FLAVOR);
	}
	...
	public boolean accept(Node elementNode) {
		try {
			...
			return name.equals(BundleElement.Type.Track.toString()) && 
				flavor.equals(PresenterTrack.FLAVOR.toString());
	...
	public boolean accept(File file, BundleElement.Type type, BundleElementFlavor flavor) throws IOException {
		if (type == null || flavor == null)
			return false;
		else if ((!type.equals(BundleElement.Type.Track) || !flavor.equals(PresenterTrack.FLAVOR)) && !checkFilenamePrefix(file, PresenterTrack.FILENAME_PREFIX))
			return false;
	...
	public BundleElement elementFromManifest(Node elementNode, File bundleRoot, boolean verify) throws BundleException {
		...
		try {
			...
			// verify the track
			if (verify) {
				log_.debug("Verifying integrity of presenter track " + trackPath);
			...
			PresenterTrackImpl track = PresenterTrackImpl.fromFile(new File(trackPath), MimeTypes.parseMimeType(mimeType), checksum);
			...
			if (audioSettingsNode != null && audioSettingsNode.hasChildNodes()) {
				try {
					AudioSettings settings = AudioSettingsImpl.fromManifest(audioSettingsNode, xpath);
			...
			throw new BundleException("Error while reading presenter track " + trackPath + ": " + e.getMessage());
		}
	}
	...
	public BundleElement elementFromFile(File file) throws BundleException {
		try {
			...
			PresenterTrackImpl track = PresenterTrackImpl.fromFile(file);
			addMediaInfo(track);
			return track;
		} catch (IOException e) {
			throw new BundleException("Error reading presenter track from " + file + " : " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new BundleException("Presenter track " + file + " has an unknown file type: " + e.getMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new BundleException("Presenter track " + file + " cannot be checksummed: " + e.getMessage());
