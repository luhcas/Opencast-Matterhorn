 ...
 * replay timeline file format and provides the functionality of reading it on behalf of
 * the bundle.
 * <p>
 ...
 * @version $Id$
 */
public class TimelineBuilderPlugin extends AbstractElementBuilderPlugin implements BundleElementBuilderPlugin {
	...
	private final static Logger log_ = LoggerFactory.getLogger(TimelineBuilderPlugin.class);
		...
		if (mimeTypes == null) {
			...
			for (String m : MIME_TYPES) {
				try {
					...
					log_.warn("Unable to create bundle timelines: mimetype " + m + " is not supported");
				}
			}
	...
	public boolean accept(File file, BundleElement.Type type, BundleElementFlavor flavor) throws IOException {
		try {
			...
			return 
				checkFilename(file, Timeline.FILENAME) &&
	...
	public BundleElement elementFromManifest(Node elementNode, File bundleRoot, boolean verify) throws BundleException {
		...
		try {
			...
			throw new ConfigurationException("Unable to calculate checksum for dublin core catalog " + catalogPath + ": " + e.getMessage());
		} catch (UnknownFileTypeException e) {
	...
	public BundleElement elementFromFile(File file) throws BundleException {
		try {
			...
			throw new BundleException("Error reading timeline from " + file + " : " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new BundleException("Timeline " + file + " has an unknown file type: " + e.getMessage());
		} catch (ParserConfigurationException e) {
			throw new BundleException("Parser configuration exception while reading timeline from " + file + " : " + e.getMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new BundleException("Timeline " + file + " cannot be checksummed: " + e.getMessage());
		} catch (SAXException e) {
			throw new BundleException("Error parsing timeline " + file + " : " + e.getMessage());
		}
	}
