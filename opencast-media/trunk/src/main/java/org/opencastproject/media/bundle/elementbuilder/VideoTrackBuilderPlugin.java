 ...
 * video tracks and provides the functionality of reading it on behalf of the
 * bundle.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id$
 */
public class VideoTrackBuilderPlugin extends AbstractTrackBuilderPlugin
    implements BundleElementBuilderPlugin {

  /** the logging facility provided by log4j */
  private final static Logger log_ = LoggerFactory
      .getLogger(VideoTrackBuilderPlugin.class);
  ...
  public boolean accept(File file, BundleElement.Type type,
    ...
    if (type != null && flavor != null) {
      if (!type.equals(BundleElement.Type.Track)
          || !flavor.equals(VideoTrack.FLAVOR))
        return false;
    ...
    return metadata != null && metadata.hasVideoStreamMetadata()
        && !metadata.hasAudioStreamMetadata();
  }
  ...
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      ...
      VideoTrackImpl track = VideoTrackImpl.fromFile(new File(trackPath),
      ...
      throw new BundleException("Error while reading video track " + trackPath
          + ": " + e.getMessage());
    }
  }
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException("Error reading video track from " + file
          + " : " + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Video track " + file
