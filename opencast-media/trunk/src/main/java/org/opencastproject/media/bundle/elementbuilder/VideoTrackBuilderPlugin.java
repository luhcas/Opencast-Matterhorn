...
import org.opencastproject.media.bundle.VideoTrack;
import org.opencastproject.media.bundle.VideoTrackImpl;
...
/**
 * This implementation of the {@link BundleElementBuilderPlugin} recognises video
 * tracks and provides the functionality of reading it on behalf of the bundle.
 * 
 * @author Tobias Wunden <tobias.wunden@id.ethz.ch>
 * @version $Id$
 */
public class VideoTrackBuilderPlugin extends AbstractTrackBuilderPlugin implements BundleElementBuilderPlugin {
	
	/** the logging facility provided by log4j */
	private final static Logger log_ = LoggerFactory.getLogger(VideoTrackBuilderPlugin.class);

    public VideoTrackBuilderPlugin() throws IllegalStateException {
        setPriority(0);
    }
	...
	public boolean accept(Type type, BundleElementFlavor flavor) {
		return type.equals(Type.Track) && flavor.equals(VideoTrack.FLAVOR);
	}
	...
	public boolean accept(Node elementNode) {
		try {
			...
			return 
				...
				flavor.equals(VideoTrack.FLAVOR.toString());
	...
	public boolean accept(File file, BundleElement.Type type, BundleElementFlavor flavor) throws IOException {
		if (type != null && flavor != null) {
			if (!type.equals(BundleElement.Type.Track) || !flavor.equals(VideoTrack.FLAVOR))
				return false;
		} else if (type != null && !type.equals(BundleElement.Type.Track)) {
			return false;
		} else if (flavor != null && !flavor.equals(VideoTrack.FLAVOR)) {
			return false;
		}
	...
	public BundleElement elementFromManifest(Node elementNode, File bundleRoot, boolean verify) throws BundleException {
		...
		try {
			...
			// verify the track
			if (verify) {
				log_.debug("Verifying integrity of video track " + trackPath);
			...
			VideoTrackImpl track = VideoTrackImpl.fromFile(new File(trackPath), MimeTypes.parseMimeType(mimeType), checksum);
			...
			throw new BundleException("Error while reading video track " + trackPath + ": " + e.getMessage());
		}
	}
	...
	public BundleElement elementFromFile(File file) throws BundleException {
		try {
			...
			VideoTrackImpl track = VideoTrackImpl.fromFile(file);
			addMediaInfo(track);
			return track;
		} catch (IOException e) {
			throw new BundleException("Error reading video track from " + file + " : " + e.getMessage());
		} catch (UnknownFileTypeException e) {
			throw new BundleException("Video track " + file + " has an unknown file type: " + e.getMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new BundleException("Video track " + file + " cannot be checksummed: " + e.getMessage());
