 ...
 * <p>
 * Expanded names in XML consists of a namespace name and a local part. In
 * opposite to <dfn>Qualified Names</dfn> - which are made from an optional
 * prefix and the local part - expanded names are <em>not</em> subject to
 * interpretation. Please see <a
 * href="http://www.w3.org/TR/xml-names/">http://www.w3.org/TR/xml-names/</a>
 * for a complete definition and reference.
 * 
...
public class EName implements Serializable {
   ...
   * 
   * @param namespaceName
   *          the name of the namespace this EName belongs to. If set to
   *          {@link javax.xml.XMLConstants#NULL_NS_URI}, this name does not
   *          belong to any namespace. Use this option with care.
   * @param localName
  ...
  /**
   * Creates a new expanded name which does not belong to a namespace. The
   * namespace name is set to {@link javax.xml.XMLConstants#NULL_NS_URI}.
   */
   ...
   * 
   * @return the namespace name or {@link javax.xml.XMLConstants#NULL_NS_URI} if
   *         the name does not belong to a namespace
   */
  ...
  public String toString() {
    return "EName{" + "namespaceName='" + namespaceName + '\''
        + ", localName='" + localName + '\'' + '}';
  }
}
