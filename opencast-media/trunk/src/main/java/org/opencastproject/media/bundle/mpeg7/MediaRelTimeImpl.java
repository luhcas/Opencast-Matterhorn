...
public class MediaRelTimeImpl extends MediaTimeImpl {
	...
	/**
	 * Creates a new media time instance, representing a time and duration that
	 * is relative to <code>referenceTimePoint</code>.
	 * 
	 * @param time the time point relative to referenceTimePoint
	 ...
	 * @param fraction the number of milliseconds
	 * @param fractionsPerSecond the number of fractions
	 * @param duration the duration
	 */
	public MediaRelTimeImpl(int hour, int minute, int second, int fraction, int fractionsPerSecond, long duration) {
		super(new MediaRelTimePointImpl(hour, minute, second, fraction, fractionsPerSecond), new MediaDurationImpl(duration));
	}
