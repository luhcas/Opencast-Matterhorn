 ...
 * @version $Id: AbstractAttachmentBuilderPlugin.java 102 2009-04-03 20:05:36Z
 *          wunden $
 */
public abstract class AbstractAttachmentBuilderPlugin extends
    AbstractElementBuilderPlugin {

  /** the logging facility provided by log4j */
  private final static Logger log_ = LoggerFactory
      .getLogger(AbstractAttachmentBuilderPlugin.class);
  ...
  /**
   * Creates a new attachment plugin builder that will accept attachments with
   * any flavor.
   */
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleElementBuilderPlugin#accept(java.io.File,
  ...
  public boolean accept(Node elementNode) {
    try {
      ...
      // Check flavor
      if (this.flavor != null) {
        String nodeFlavor = (String) xpath.evaluate("@type", elementNode,
      ...
      // Check mime type
      if (mimeTypes != null && mimeTypes.size() > 0) {
        String nodeMimeType = (String) xpath.evaluate("MimeType", elementNode,
            XPathConstants.STRING);
        try {
      ...
      log_.warn("Error while reading attachment flavor from manifest: "
          + e.getMessage());
      return false;
    }
  }
  ...
  /**
   * @see org.opencastproject.media.bundle.BundleElementBuilder#newElement(org.opencastproject.media.bundle.BundleElement.Type,
   *      org.opencastproject.media.bundle.BundleElementFlavor)
   */
  public BundleElement newElement(BundleElement.Type type,
      BundleElementFlavor flavor) {
    throw new UnsupportedOperationException(
        "Creation of new attachments from scratch is unsupported");
  }
  ...
  public BundleElement elementFromManifest(Node elementNode, File bundleRoot,
    ...
    try {
      // id
      attachmentId = (String) xpath.evaluate("@id", elementNode,
          XPathConstants.STRING);

      // flavor
      attachmentFlavor = (String) xpath.evaluate("@type", elementNode,
          XPathConstants.STRING);
      ...
      attachmentPath = PathSupport.concat(bundleRoot.getAbsolutePath(),
          attachmentPath);

      // create the attachment
      AttachmentImpl attachment = AttachmentImpl.fromFile(new File(
          attachmentPath));
      ...
      if (attachmentFlavor != null && !attachmentFlavor.equals("")) {
        try {
          BundleElementFlavor flavor = BundleElementFlavor
              .parseFlavor(attachmentFlavor);
      ...
      throw new BundleException(
          "Error while reading attachment from manifest: " + e.getMessage());
    } catch (NoSuchAlgorithmException e) {
      throw new BundleException("Unsupported digest algorithm: "
          + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new ConfigurationException("Filetype not supported: "
          + e.getMessage());
    } catch (IOException e) {
      throw new BundleException("Error while reading attachment file "
  ...
  public BundleElement elementFromFile(File file) throws BundleException {
    try {
      ...
      throw new BundleException("Error reading dublin core from " + file
          + " : " + e.getMessage());
    } catch (UnknownFileTypeException e) {
      throw new BundleException("Dublin core metadata " + file
          + " has an unknown file type: " + e.getMessage());
    } catch (NoSuchAlgorithmException e) {
      throw new BundleException("Dublin core document " + file
   ...
   *      org.opencastproject.media.bundle.BundleElementFlavor)
   */
  public BundleElement elementFromFile(File file, BundleElement.Type type,
   ...
   * attachment. This implementation just returns the attachment that is was
   * given.
   * 
   * @param attachment
   ...
   * @throws BundleException
   *           if the bundle fails to be specialized
   */
  protected Attachment specializeAttachment(Attachment attachment)
      throws BundleException {
    return attachment;
  }
